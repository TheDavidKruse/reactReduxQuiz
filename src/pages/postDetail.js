...
class PostDetail extends Component {
  ...
  setCurrentPostDetails(incomingProps){
    ...
    if(location.state){
      ...
      if(currentComments.length < incom.length){
        console.log(incom)
      ...
      console.log("Setting the state the standard way")
  ...
  render () {
    let { post, author, comments, } = this.state
      if(Object.keys(this.state).length === 1 && this.state.foundPost === false){
        ...
        console.log("THIS.STATE, THISPROPS",this.state, this.props)
        return (
          <Container>
            <Grid>
              <Grid.Row>
                <Image style={{width:'100vw'}} src={banner}/>
              </Grid.Row>
              <Grid.Row>
